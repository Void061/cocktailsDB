            ...
            <ul className="flex justify-center flex-wrap gap-4">
                {props.categories.map((item, index) => (
                    ...
                    <Link onClick={() => handleChangeCat(encodeCategory(item.strCategory))} href={'/category/'+ encodeCategory(item.strCategory)} key={index} scroll={false}>
                        <li className="transition-all hover:bg-primary hover:text-secondary text-[20px] font-medium px-6 cursor-pointer rounded-[25px] text-primary bg-secondary" >{item.strCategory}</li>
                        
                    </Link> :
                    <li key={index} className="transition-all text-[20px] font-medium px-6 cursor-pointer rounded-[25px] text-secondary bg-primary" >{item.strCategory}</li>                 
                ))}
            </ul>
